pipeline {
  ...
  stages {
    ...
    stage('Security scan') {
      steps {
        sh '''
          trivy image --exit-code 0 --severity HIGH,CRITICAL ${IMAGE_PREFIX}/api-gateway:${BUILD_NUMBER}
          trivy image --exit-code 0 --severity HIGH,CRITICAL ${IMAGE_PREFIX}/orders-service:${BUILD_NUMBER}
        '''
      }
    }
